...
factorial:			# Procedure named Factorial
	...
	ori $v0, $zero, 1	# (for cae $a0<1) set as $v0=1
	jr $ra			# return to caller

	L1:			# (for case $a0>1)
	addi $sp, $sp, -12	# Move stack pointer downward by 3 word
	sw $fp, 8($sp)		# backup the value of $fp, (push into stack)
	sw $ra, 4($sp)		# backup the value of $ra, (push into stack)
	sw $a0, 0($sp)		# backup the value of $a0, (push into stack)
	addi $fp, $sp, 8	# Set new frame
	...
	lw $fp, 8($sp)		# restore the value of $fp, (pop from stack)
	addi $sp, $sp, 12	# Move stack pointer upward by 3 word
